...
let screenDIM = (Platform.OS === 'web') ? {  height: window.innerHeight, width: window.innerWidth, } : Dimensions.get('window');
const isAndroid = Platform.OS === 'android'
const toolbarHeight = isAndroid ? 25 : 0

let activePosition = { x: screenDIM.width, y: screenDIM.height }
...
const calcSide = (side) => {
  let y
  let x
  screenDIM = (Platform.OS === 'web') ? {  height: window.innerHeight, width: window.innerWidth, } : Dimensions.get('window');
  // console.log('calcSide', { screenDIM });
  activePosition = { x: screenDIM.width, y: screenDIM.height }
...
export const scene = (opts = {}) => (Wrap) => {
  ...
  class Scene extends Component {
    ...
    _handleStartShouldSetPanResponder = (evt, gestureState) => {
      ...
      switch (side) {
        case Side.B:
          ...
          break
        case Side.T:
          if (screenDIM.height - threshold > pageY) {
            return false
          }
          break
        case Side.L:
          if (screenDIM.width - threshold > pageX) {
            return false
          }
          break
        case Side.R:
    ...
    render() {
      ...
      // console.log('Scene RENDER &&&&&& calcSide', { screenDIM });
